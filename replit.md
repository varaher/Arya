...
-   **Indian Classical Arts & Music Knowledge:** `arya_knowledge` (domain `sanskrit`) includes the two most authoritative ancient Indian treatises on music/performing arts: Natyashastra (Bharata Muni, ~200 BCE–200 CE — the foundational integrated-arts text covering drama, dance, and music together; source of Rasa/Navarasa theory, Sapta Swara, Grama-Murchana, and the Tala/instrument classification systems) and Sangita Ratnakara (Sharngadeva, 13th century — the definitive purely music-focused treatise on swara, 264 ragas, 120 talas, and Prabandha song-structure; common ancestor of both Hindustani and Carnatic classical traditions). Seeded via `server/seed-music-shastras.ts`.
