...
-   **Niti — The Wisdom Council** (`niti.ts`): Business decision-making module. 6-screen dark-premium PWA (`/niti`). Philosopher routing: Chanakya (strategy/competition), Vidura (ethics/hiring/trust), Thiruvalluvar (resilience/timing), Krishna/Gita (founder mindset/pressure), Shukracharya (finance/structure). ARYA selects the philosopher automatically based on session type + message content + focus areas. Each response has structured format: insight + push question (italic gold, gets under the surface) + 3 branching follow-up chips. Session types: Help me decide / Stress-test my plan / People situation / Think out loud. Sessions stored in `arya_niti_sessions` + `arya_niti_messages`. User business profile stored on `arya_users` (nitiEnabled, businessType, businessStage, businessRole, businessChallenge, businessFocusAreas). Routes: GET /api/niti/profile, POST /api/niti/setup, POST /api/niti/sessions, GET /api/niti/sessions, POST /api/niti/sessions/:id/message. Menu: ✦ Niti — Business Wisdom in both user menus.
